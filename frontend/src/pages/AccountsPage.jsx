...
const DEPARTMENT_NAME = "ACCOUNTS";
...
const normalizeRecord = (record, index) => ({
  ...
  contractName: record.accountsMeta?.contractName || "",
  account: record.accountsMeta?.account || "",
  category: record.accountsMeta?.category || "",
  activity: record.accountsMeta?.activity || "",
...
const AccountsModal = ({ open, onClose, onSubmit, record, nextSrNo }) => {
  ...
  useEffect(() => {
    ...
    if (record) {
      setForm({
        contractName: record.contractName || CONTRACT_OPTIONS[0],
        account: record.account || ACCOUNT_OPTIONS[0],
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4">
      <div className="mx-auto max-w-5xl rounded-lg bg-card p-6 shadow-soft">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-primary/70">Accounts Workflow</p>
            <h3 className="mt-2 font-display text-3xl text-body">{record ? "Edit Accounts Row" : "Add Accounts Row"}</h3>
          </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <label className="block space-y-2 text-sm font-medium text-body">
                  ...
                  <input
                    type="text"
                    value={form.revision}
                    onChange={(event) => setForm((current) => ({ ...current, revision: event.target.value }))}
                    className="w-full rounded-lg border border-border px-4 py-3 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10"
                  />
                </label>
                <label className="block space-y-2 text-sm font-medium text-body">
...
const AccountsPage = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to load accounts records.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRecords = useMemo(() => {
    ...
    return records
      .filter((record) => contractFilter === "ALL" || record.contractName === contractFilter)
      .filter((record) => accountFilter === "ALL" || record.account === accountFilter)
  ...
  }, [records, contractFilter, accountFilter, categoryFilter, search]);
  ...
  const handleSave = async (form, srNo) => {
    try {
      ...
      if (selectedRecord) {
        ...
        toast.success("Accounts row updated.");
      } else {
        await recordsApi.create(payload);
        toast.success("Accounts row added.");
      }
      ...
      toast.error(error.response?.data?.message || "Unable to save accounts row.");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Accounts row deleted.");
      await loadRecords();
    } catch (error) {
      toast.error(error.response?.data?.message || "Unable to delete accounts row.");
    }
  };
  ...
  const handleExportCsv = () => {
    ...
    const rows = filteredRecords.map((record) =>
      [record.srNo, record.contractName, record.activity, record.document, record.revision, record.status]
        .map(csvEscape)
        .join(",")
    );

    downloadBlob([headers.join(","), ...rows].join("\n"), "accounts-records.csv", "text/csv;charset=utf-8;");
  };

  const handleDownloadExcel = () => {
    downloadBlob(
      buildExcelTableMarkup(filteredRecords),
      "accounts-records.xls",
      "application/vnd.ms-excel;charset=utf-8;"
    );
  };

  return (
    <AppShell searchValue="" onSearchChange={() => {}} searchPlaceholder="Use accounts filters below." searchDisabled>
      <PageHeader
        eyebrow="Accounts Workflow"
        title="ACCOUNTS"
        description="Filter accounts records by contract, account, and category, then maintain row-based document workflow with export and download tools."
        action={
        ...
        <label className="space-y-2 text-sm font-medium text-body">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-border px-4 py-3">
            ...
            <input
              ...
              placeholder="Search contract, category, activity, document, revision, or status"
              className="w-full border-none bg-transparent text-sm outline-none"
            />
          </div>
        </label>
      ...
      {loading ? (
        <Spinner label="Loading accounts rows..." />
      ) : (
                ...
                {filteredRecords.length ? (
                  ...
                  <tr>
                    <td colSpan="7" className="px-4 py-16 text-center text-sm text-slate-500">
                      No accounts rows available for the current filters.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {canDelete ? (
        <ConfirmationModal
          ...
          title="Delete accounts row?"
          description="This moves the selected accounts row to the admin restore bin."
